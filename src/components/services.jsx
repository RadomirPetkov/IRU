      ...
      <div className="container mx-auto px-4 max-w-[1500px]">
          ...
          <div className="max-w-full mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
            <div className="p-8 md:p-12">
              <div className="flex flex-col md:flex-row items-center mb-8">
                <div className="bg-blue-50 p-6 rounded-xl border-l-4 border-blue-500 mb-6 md:mb-0 md:mr-8 md:w-2/5 flex flex-col justify-center">
                  <span className="italic block text-xl text-blue-700 mb-4 leading-relaxed">
                    "Технологията е просто инструмент. От гледна точка на това да накараме хората да работят заедно и да ги мотивираме, учителят е най-важен."
                  </span>
                  <span className="font-semibold block text-gray-600 text-right">— Бил Гейтс</span>
                </div>
                
                <div className="md:w-3/5">
                  <h3 className="text-xl font-bold text-gray-800 mb-4">За нашите курсове</h3>
                  <p className="text-gray-600 mb-4">
                    Ние провеждаме <span className="font-semibold text-blue-700">безплатни и изцяло дистанционни</span> курсове за цифрови
                    компетентности, които се провеждат в делнични дни между <span className="font-semibold">18:00 и
                    21:00 ч.</span>
                  </p>
              ...
              <div className="flex flex-wrap justify-center items-center pt-6 border-t border-gray-100">
                <div className="px-4 py-2 m-2 bg-green-50 rounded-lg flex items-center">
                ...
                <div className="px-4 py-2 m-2 bg-purple-50 rounded-lg flex items-center">
                  ...
                  <span className="text-gray-700">За съвременния дигитален свят</span>
                </div>
                <div className="px-4 py-2 m-2 bg-yellow-50 rounded-lg flex items-center">
                  <div className="w-3 h-3 bg-yellow-500 rounded-full mr-2"></div>
                  <span className="text-gray-700">За всички нива на познания</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          {props.data ? props.data.map((course, i) => (
            ...
            >
              <div 
                className={`flex items-center justify-between p-6 cursor-pointer ${expandedCourse === i ? 'bg-blue-50' : 'bg-white'}`}
                onClick={() => toggleCourse(i)}
              >
                <div className="text-center">
                <h3 className="text-xl md:text-2xl font-bold text-gray-800">{course.name}</h3>
                <p className="text-blue-600 mt-1 font-medium">
                  {i === 0 ? "Цифрова компетентност - Базово ниво" : "Цифрова компетентност - Средно ниво"}
                </p>
              </div>
                <div className={`bg-blue-500 rounded-full p-2 text-white transition-transform duration-300 ${expandedCourse === i ? 'rotate-180' : ''}`}>
                  <ChevronDown size={20} />
                </div>
              </div>
                ...
                <div className="p-6 bg-gradient-to-b from-blue-50 to-white">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-hidden">
                      <div 
                        className={`flex items-center justify-between p-4 cursor-pointer ${expandedSection[`${i}-learning`] ? 'bg-green-50' : 'bg-white'}`}
                      ...
                      >
                        ...
                        <div className={`transition-transform duration-300 ${expandedSection[`${i}-learning`] ? 'rotate-180' : ''}`}>
                      ...
                      {expandedSection[`${i}-learning`] && (
                        <div className="p-4 bg-gradient-to-b from-green-50 to-white">
                          <ul className="space-y-3">
                            {course.learning && course.learning.map((item, idx) => (
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-hidden">
                      <div 
                        className={`flex items-center justify-between p-4 cursor-pointer ${expandedSection[`${i}-conditions`] ? 'bg-blue-50' : 'bg-white'}`}
                      ...
                      >
                        ...
                        <div className={`transition-transform duration-300 ${expandedSection[`${i}-conditions`] ? 'rotate-180' : ''}`}>
                          <ChevronDown size={18} className="text-gray-500" />
                        </div>
                      </div>
                      ...
                      {expandedSection[`${i}-conditions`] && (
                        <div className="p-4 bg-gradient-to-b from-blue-50 to-white">
                          <ul className="space-y-3">
                            {course.conditions && course.conditions.map((item, idx) => (
                              <li key={idx} className="flex items-start">
                                <div className="mt-1 mr-3 flex-shrink-0">
                    ...
                    <div className="bg-white rounded-lg shadow-md overflow-hidden">
                      <div 
                        className={`flex items-center justify-between p-4 cursor-pointer ${expandedSection[`${i}-signup`] ? 'bg-indigo-50' : 'bg-white'}`}
                      ...
                      >
                        ...
                        <div className={`transition-transform duration-300 ${expandedSection[`${i}-signup`] ? 'rotate-180' : ''}`}>
                          <ChevronDown size={18} className="text-gray-500" />
                        </div>
                      </div>
                      ...
                      {expandedSection[`${i}-signup`] && (
                        <div className="p-4 bg-gradient-to-b from-indigo-50 to-white">
                          <ul className="space-y-3">
                            {course.signup && course.signup.map((item, idx) => (
                              <li key={idx} className="flex items-start">
                                <div className="mt-1 mr-3 flex-shrink-0">
                                  {idx <= 1 ? (
